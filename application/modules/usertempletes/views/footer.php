...
<footer class="main-footer">
    ...
    <strong>Copyright &copy; 2017 <a href="http://mondaymorning.co.in">Monday Morning</a>.</strong> All rights
    reserved.
  </footer>
...
//########## Code by Ravi  ######################
	$("#faoccupation").keypress(function (event) {
    ...
    if (event.keyCode === 8 || event.keyCode === 46) {
        return true;
    } else if ( key < 65 || key > 127 ) {
		alert('Write only the alphabets');
        return false;
    } else {
        return true;  
    }
	...
	$("#mooccupation").keypress(function (event) {
	...
	$("#spoccupation").keypress(function (event) {
    ...
    if (event.keyCode === 8 || event.keyCode === 46) {
		...
		$('#addressnotnumber').focusout(function(){
			var add=$('#addressnotnumber').val();
			 var numericReg = /^\d*[0-9](|.\d*[0-9]|,\d*[0-9])?$/;
			 if(!numericReg.test(add)) {
				return true;
			 }
			 else{
				 alert("Address May Not Contain Only Number");
				 $("#addressnotnumber").val("");
			 }
			
		});
		$('#addressnotnumberA').focusout(function(){
			var addA=$('#addressnotnumberA').val();
			 var numericRegA = /^\d*[0-9](|.\d*[0-9]|,\d*[0-9])?$/;
			 if(!numericRegA.test(addA)) {
				return true;
			 }
			 else{
				 alert("Address May Not Contain Only Number");
				 $("#addressnotnumberA").val("");
			 }
			
		});
			
		$('#addressnotnumberB').focusout(function(){
			var addB=$('#addressnotnumberB').val();
			//alert(addB);
			 var numericRegB = /^\d*[0-9](|.\d*[0-9]|,\d*[0-9])?$/;
			 if(!numericRegB.test(addB)) {
				return true;
			 }
			 else{
				 alert("Address May Not Contain Only Number");
				 $("#addressnotnumberB").val("");
			 }
			
		});	
		$('#ref2_add').focusout(function(){
			var refadd2=$('#ref2_add').val();
			//alert(addB);
			 var refnumericReg2 = /^\d*[0-9](|.\d*[0-9]|,\d*[0-9])?$/;
			 if(!refnumericReg2.test(refadd2)) {
				return true;
			 }
			 else{
				 alert("Address May Not Contain Only Number");
		...
		$('#ref1_add').focusout(function(){
			var refadd1=$('#ref1_add').val();
			//alert(addB);
			 var refnumericReg1 = /^\d*[0-9](|.\d*[0-9]|,\d*[0-9])?$/;
			 if(!refnumericReg1.test(refadd1)) {
				return true;
			 }
			 else{
				 alert("Address May Not Contain Only Number");
		...
		$("#xmark").focusout(function(){
			var  markper=$("#xmark").val();
			if(markper<100)
			{
				return true;
			}
			...
			$("#xiimark").focusout(function(){
			var  Xiimarkper=$("#xiimark").val();
			if(Xiimarkper<100)
			{
				return true;
			}
			...
			$("#gmark").focusout(function(){
			var  markper=$("#gmark").val();
			if(markper<100)
			{
				return true;
			}
		...
		$("#pgm").focusout(function(){
			var  pgmarkper=$("#pgm").val();
			if(pgmarkper<100)
			{
				return true;
			}
		...
		$("#xiiyear").change(function(){
			var xyearvalu=parseInt($("#xyear").val());
				xyear =xyearvalu+2; 
			var xiiyear=parseInt($("#xiiyear").val());
			
			if(xyear<=xiiyear)
				return true;
			else
				alert("Select Year is Greater Then X Pass Year");
				$('#xiiyear').prop('selectedIndex',0);
			
		});
		
			$("#gyear").change(function(){
			
			var xiiyearp=parseInt($("#xiiyear").val());
			var gyearvalu=parseInt($("#gyear").val());
			if(xiiyearp<gyearvalu)
				return true;
			else
				alert("Select Year is Greater Then XII Pass Year");
				$('#gyear').prop('selectedIndex',0);
